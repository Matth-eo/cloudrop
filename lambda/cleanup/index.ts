import { DeleteItemCommand, DynamoDBClient, GetItemCommand, ScanCommand, type AttributeValue } from "@aws-sdk/client-dynamodb";
import { DeleteObjectCommand, GetBucketVersioningCommand, ListObjectVersionsCommand, S3Client } from "@aws-sdk/client-s3";
import { getFileId } from "../../src/lib/file-key";
...
async function deleteFile(bucket: string, key: string, versioned: boolean) {
  if (!versioned) {
  ...
  try {
    ...
    do {
      ...
      for (const candidate of page.Items ?? []) {
        ...
        if (!fileId) { summary.skipped++; continue; }
        try {
          ...
          if (expiresAt > cutoff) { summary.skipped++; continue; }
